...
template <typename Dtype>
    void SpatialTransformerLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
        ...
        if (bottom.size() == 2) {
            ...
            CHECK(pre_defined_count == 6) << "The dimension of theta is not six!"
                << " Only " << pre_defined_count << std::endl;
        }
        ...
        shape_input[0] = bottom[0]->shape(0); shape_input[1] = output_H_ * output_W_; shape_input[2] = 2;
...
template <typename Dtype>
    void SpatialTransformerLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
        ...
        CHECK(false) << "There remain bug in CPU implementation. Don't use!" << std::endl;
