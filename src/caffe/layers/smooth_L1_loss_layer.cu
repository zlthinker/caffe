...
template <typename Dtype>
    void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
        ...
        Dtype* tmp_weights = weights_.mutable_cpu_data();
        int batch_size = bottom[1]->shape(0);
        int vec_dimension = bottom[1]->count() / batch_size;
        // no reproduction required
        /*std:: cout << batch_size << std:: endl;*/
        /*std:: cout << vec_dimension << std::endl;*/
        if (reproduce_ == 1) {
            caffe_copy(batch_size * vec_dimension, bottom[1]->gpu_data(), weights_.mutable_gpu_data());
        } else {
            for (int v = 0; v < batch_size; ++v) {
                for (int w = 0; w < vec_dimension; w++) {
                    // prevent negative weights
                    Dtype tmp_label = std::max(Dtype(0), *(bottom[1]->cpu_data() + v * vec_dimension + w));
                    caffe_set(reproduce_, tmp_label,
                            weights_.mutable_cpu_data() + v * vec_dimension + w * reproduce_);
                }
                /*for (int w = 0; w < vec_dimension * reproduce_; w++)*/
                    /*std::cout << weights_.cpu_data()[w] << ' ';*/
                /*std::cout << std::endl;*/
            }
        }
        caffe_gpu_sub(
