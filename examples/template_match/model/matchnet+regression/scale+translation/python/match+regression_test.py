...
def readFileList(filepath, label_num):
    file_list = []
    labels = []
...
def visualizePair(img1, img2, prob, regression, gt_boxes, save_path):
    ...
    center_x = width * 3 / 2
    center_y = height / 2
    x1 = width * regression[0] + width
    y1 = height * regression[1]
    x2 = width * regression[2] + width
    y2 = height * regression[3]
    ax.add_patch(plt.Rectangle((x1, y1),
                    x2 - x1, y2 - y1, fill=False, edgecolor='green', linewidth=3.5))

    gt_x1 = width * float(gt_boxes[0]) + width
    gt_y1 = height * float(gt_boxes[1])
    gt_x2 = width * float(gt_boxes[2]) + height
    gt_y2 = height * float(gt_boxes[3])
    ax.add_patch(plt.Rectangle((gt_x1, gt_y1),
                    gt_x2 - gt_x1, gt_y2 - gt_y1, fill=False, edgecolor='red', linewidth=3.5))
...
if __name__ == '__main__':
    ...
    template_list, gt_boxes = readFileList(args.template_list, 4)
    search_list, label_list = readFileList(args.search_list, 1)
    ...
    for i in range(test_num):
        ...
        if softmax_prob[0] < softmax_prob[1]:
            confidence = 1
        result = 0
        if confidence == int(label_list[i][0]):
            result = 1
        true_num += result
        print 'Result: ', result
