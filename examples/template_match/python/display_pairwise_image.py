...
if __name__ == '__main__':
    ...
    parser.add_argument('image_list_p', help='pairwise image list')
    args=parser.parse_args()

    image_list_path = args.image_list
    image_list_path_p = args.image_list_p

    if not os.path.isfile(image_list_path):
        raise IOError('File not exists:' + image_list_path)
    if not os.path.isfile(image_list_path_p):
        raise IOError('File not exists:' + image_list_path_p)

    image_list, label0 = readFileList(image_list_path)
    image_list_p, label1 = readFileList(image_list_path_p)
    image_num = min(len(image_list), len(image_list_p))
    ...
    for i in range(image_num):
        ...
        image_p = caffe.io.load_image(image_list_p[i])
        concat = concatPairwiseImages(image, image_p, label1[i])
        ax.imshow(concat)
        fig.canvas.draw()
